...
import com.mysoft.admin.service.MenuService;
...
public class UrlFilterInvocationSecurityMetadataSource implements FilterInvocationSecurityMetadataSource {
    ...
    public Collection<ConfigAttribute> getAttributes(Object o) throws IllegalArgumentException {
        ...
        List<MenuRoleBean> allMenu = menuService.getAllMenu();
        for (MenuRoleBean menu : allMenu) {
            if (antPathMatcher.match(menu.getUrl(), requestUrl) && menu.getRoleList().size()>0) {
                List<RoleBean> roles = menu.getRoleList();
                int size = roles.size();
